...
const users = [{ id: 1, usuario: 'test', password: 'test', activo: true, token: null }];
let FakeBackendInterceptor = class FakeBackendInterceptor {
    intercept(request, next) {
        ...
        function authenticate() {
            ...
            const user = users.find(x => x.usuario === username && x.password === password);
            if (!user)
...
let AuthenticationService = class AuthenticationService {
    ...
    login(username, password) {
        let headers = { "Authorization": "Basic " + btoa("macucoapp:secret") };
        let params = { 'grant_type': 'password', 'username': username, 'password': password };
        return this.http.post(`${src_environments_environment_prod__WEBPACK_IMPORTED_MODULE_6__["environment"].apiUrl}/oauth/token`, null, { headers, params })
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["map"])(token => {
            // store user details and jwt token in local storage to keep user logged in between page refreshes
            let user = new _models__WEBPACK_IMPORTED_MODULE_5__["Usuario"]();
            user.token = token;
...
let DispositivosService = class DispositivosService {
    ...
    getAll() {
        return this.http.get(`${src_environments_environment_prod__WEBPACK_IMPORTED_MODULE_4__["environment"].apiUrl}/control/dispositivos`)
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])(dispositivos => {
            dispositivos.forEach(disp => { disp.dispositivoId = disp.id.dispositivoId; });
            return dispositivos;
        }));
    }
};
DispositivosService.ctorParameters = () => [
    { type: _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpClient"] }
];
...
class WebsocketService {
    ...
    connect(usuario) {
        ...
        const socket = new sockjs_client__WEBPACK_IMPORTED_MODULE_2__(`${src_environments_environment_prod__WEBPACK_IMPORTED_MODULE_5__["environment"].websocketUrl}?access_token=${this.currentUser.token.access_token}`);
        const _this = this;
...
let AppComponent = class AppComponent {
    constructor(router, authenticationService, websocket) {
        this.router = router;
        this.authenticationService = authenticationService;
...
let DispositiveComponent = class DispositiveComponent {
    constructor(auth, websocket) {
        this.auth = auth;
        this.websocket = websocket;
        this.sinSombra = true;
        this.loading = false;
    ...
    suscribierse() {
        this.websocket.subscribe(`/user/thing/${this.dispositivo.dispositivoId}/reported`).subscribe(reportedStatus => {
            this.reported(reportedStatus);
        }, err => this.disconnect(err));
        this.websocket.subscribe(`/thing/${this.dispositivo.dispositivoId}/reported`).subscribe(reportedStatus => {
            this.reported(reportedStatus);
        }, err => this.disconnect(err));
        this.websocket.subscribe(`/user/thing/errors`).subscribe(reportedStatus => {
            this.error(reportedStatus);
        }, err => this.disconnect(err));
        this.websocket.subscribe(`/user/thing/${this.dispositivo.dispositivoId}/desired/response`).subscribe(reportedStatus => {
            //console.log(reportedStatus.body);
            this.showGreeting(reportedStatus.body);
        }, err => this.disconnect(err));
    }
    reported(reportedStatus) {
        ...
        if (reportedStatus.body.includes("state")) {
            let mensaje = JSON.parse(reportedStatus.body);
            console.log(reportedStatus.body);
    ...
    changeStatus() {
        ...
        let statusTemp = this.dispositivo.status;
        setTimeout(() => {
            console.warn("Revisar status", this.dispositivo.status, this.sinSombra);
        ...
        if (this.dispositivo.status == "DESIRED") { //Random entre los status para sincronizarse
            let status = ["ON", "OFF"][Math.floor(Math.random() * 2)];
    ...
    connectar() {
        this.websocket.connect(this.auth.currentUserValue).then(ws => {
            console.log("Componente conectado");
            this.suscribierse();
            this.websocket.updateStatus(this.dispositivo);
        }).catch(error => {
            console.log("No se pudo conectar el componente", error);
            this.showGreeting("No se pudo conectar al servidor, revise sus datos");
        });
    }
    ngOnInit() {
...
DispositiveComponent.ctorParameters = () => [
    { type: _services__WEBPACK_IMPORTED_MODULE_2__["AuthenticationService"] },
...
let HomeComponent = class HomeComponent {
    ...
    ngOnInit() {
        this.loading = true;
        this.dispositivosService.getAll().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["first"])()).subscribe(disp => {
            this.loading = false;
